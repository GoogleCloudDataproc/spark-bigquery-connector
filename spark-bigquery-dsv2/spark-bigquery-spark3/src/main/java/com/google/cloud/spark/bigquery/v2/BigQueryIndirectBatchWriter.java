...
public class BigQueryIndirectBatchWriter extends GenericBigQueryIndirectDataSourceWriter implements BatchWrite{
    private static final Logger logger =
            LoggerFactory.getLogger(BigQueryIndirectBatchWriter.class);

    private final StructType sparkSchema;
    private final SaveMode saveMode;
    private final Optional<IntermediateDataCleaner> intermediateDataCleaner;

    public BigQueryIndirectBatchWriter(BigQueryClient bigQueryClient,
    ...
    public DataWriterFactory createBatchWriterFactory(PhysicalWriteInfo var1){
        ...
        return new BigQueryIndirectDataWriterFactory( new SerializableConfiguration(getHadoopConfiguration()),getGcsPath().toString(),sparkSchema,avroSchema.toString());
    }
    ...
    public void onDataWriterCommit(WriterCommitMessage message) {
        logger.info("Data for {} has been successfully written to GCS",((BigQueryIndirectWriterCommitMessage) message).getUri());
