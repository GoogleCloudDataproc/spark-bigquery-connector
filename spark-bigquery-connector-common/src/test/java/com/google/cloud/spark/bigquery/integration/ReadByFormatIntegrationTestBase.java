...
public class ReadByFormatIntegrationTestBase extends SparkBigQueryIntegrationTestBase {
  ...
  // List<Row> result = df.select("corpus").filter("word =
  // 'spark'").collectAsList();
  //
  ...
  // List<Row> result = cachedDF.select("corpus").filter("word =
  // 'spark'").collectAsList();
  //
  // assertThat(result).hasSize(9);
  // List<Row> filteredResult =
  ...
  //
  // long idealPartitionSize = TestConstants.LARGE_TABLE_NUM_ROWS /
  ...
  // assumeTrue("user provided schema is not allowed for this connector",
  // userProvidedSchemaAllowed);
  ...
  //
  // Dataset<Row> df =
  // spark.read().format("bigquery").load(String.format("%s.%s", testDataset,
  // testTable));
  ...
  // .isEqualTo(DataTypes.createMapType(DataTypes.StringType,
  // DataTypes.LongType));
  // List<Row> rowList = df.collectAsList();
  // assertThat(rowList).hasSize(2);
  // List<Map<?, ?>> result =
  // rowList.stream().map(row ->
  // scalaMapToJavaMap(row.getMap(0))).collect(Collectors.toList());
  // assertThat(result).contains(ImmutableMap.of("a", Long.valueOf(1), "b",
  // Long.valueOf(2)));
  ...
  // BigQuery bigQuery = IntegrationTestUtils.getBigquery();
  // LocalDateTime dateTime = LocalDateTime.of(2023, 9, 18, 14, 30, 15, 234 *
  // 1_000_000);
  // bigQuery.create(
  // TableInfo.newBuilder(
  // TableId.of(testDataset.toString(), testTable),
  // StandardTableDefinition.of(Schema.of(Field.of("foo",
  // LegacySQLTypeName.DATETIME))))
  // .build());
  // IntegrationTestUtils.runQuery(
  // String.format(
  // "INSERT INTO %s.%s (foo) VALUES " + "('%s')", testDataset, testTable,
  // dateTime));
  //
  // Dataset<Row> df =
  // spark
