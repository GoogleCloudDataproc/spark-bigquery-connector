...
public class ArrowBinaryRowIterator implements Iterator<Row> {

  private static long maxAllocation = Long.MAX_VALUE;
  ArrowReaderRowIterator arrowReaderRowIterator;
  Iterator<InternalRow> currentIterator;
  List<String> columnsInOrder;
  Map<String, StructField> userProvidedFieldMap;

  public ArrowBinaryRowIterator(
      List<String> columnsInOrder,
      ByteString schema,
    ...
    arrowReaderRowIterator = new ArrowReaderRowIterator(arrowStreamReader);
  ...
  public boolean hasNext() {
    while (!currentIterator.hasNext()) {
      if (!arrowReaderRowIterator.hasNext()) {
        return false;
      }
      currentIterator = toArrowRows(arrowReaderRowIterator.next(), columnsInOrder);
    }
  ...
  @Override
  public Row next() {
    return (Row) currentIterator.next();
  }
...
class ArrowReaderRowIterator implements Iterator<VectorSchemaRoot> {
  ...
  public ArrowReaderRowIterator(ArrowReader reader) {
    this.reader = reader;
  }
