...
class SparkFilterUtils {
    ...
    static String compileFilter(Filter filter) {
        ...
        if (filter instanceof StringStartsWith) {
            ...
            return format("%s LIKE '%%%s'", quote(stringStartsWith.attribute()), compileValue(stringStartsWith.value()));
        }
        if (filter instanceof StringEndsWith) {
            StringEndsWith stringEndsWith = (StringEndsWith) filter;
            return format("%s LIKE '%s%%'", quote(stringEndsWith.attribute()), compileValue(stringEndsWith.value()));
        }
        if (filter instanceof LessThan) {
            StringContains stringContains = (StringContains) filter;
            return format("%s LIKE '%%%s%%'", quote(stringContains.attribute()), compileValue(stringContains.value()));
        }
