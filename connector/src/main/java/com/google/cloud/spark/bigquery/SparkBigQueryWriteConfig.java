...
public class SparkBigQueryWriteConfig implements BigQueryConfig {
    ...
    int maxWriteRowsRetries = 3;

    private SparkBigQueryWriteConfig() {
        // empty
    }

    public static SparkBigQueryWriteConfig from(
            StructType sparkSchema,
            SaveMode saveMode,
        ...
        SparkBigQueryWriteConfig config = new SparkBigQueryWriteConfig();
        ...
        config.sparkSchema = sparkSchema;
        config.bigQuerySchema = toBigQuerySchema(sparkSchema);
        try {
            config.schemaDescriptor = toDescriptor(sparkSchema);
        } catch (Descriptors.DescriptorValidationException e) {
            throw new IllegalArgumentException("Could not infer protobuf descriptor from Spark schema.", e); // TODO: verify correct error signature
        }
        config.protoSchema = ProtoSchemaConverter.convert(config.schemaDescriptor);
        config.saveMode = saveMode;
        config.writeUUID = writeUUID;

        config.tableInfo =
                TableInfo.newBuilder(
                        config.tableId,
                        StandardTableDefinition.of(
                                config.bigQuerySchema))
                        .build();
    ...
    public boolean isViewsEnabled() {
        throw new UnsupportedOperationException("Write sessions do not make use of VIEWS.");
    }
    ...
    public int getMaxWriteRowsRetries() {
        return maxWriteRowsRetries;
    }

    public WriteSessionConfig toWriteSessionCreatorConfig() {
        return new WriteSessionConfig(
                /*viewsEnabled,
