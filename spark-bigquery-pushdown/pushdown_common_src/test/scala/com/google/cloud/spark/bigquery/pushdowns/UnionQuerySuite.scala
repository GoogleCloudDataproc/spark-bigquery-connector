...
class UnionQuerySuite extends AnyFunSuite{
  ...
  test(testName = "getStatement with nonempty children and useAlias true") {
    val unionQuery = UnionQuery(expressionConverter, expressionFactory, Seq(sourceQuery1, sourceQuery2), Some(Seq()), Some(Seq()), SUBQUERY_2_ALIAS)
  ...
  test(testName = "getStatement with empty children") {
    val unionQuery = UnionQuery(expressionConverter, expressionFactory, Seq.empty, Some(Seq()), Some(Seq()), SUBQUERY_2_ALIAS)
    val bigQuerySQLStatement = unionQuery.getStatement()
    assert(bigQuerySQLStatement.toString == "")
  }
